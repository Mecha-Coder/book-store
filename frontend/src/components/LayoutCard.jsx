      ...
      {book.map(item=>(
        <div key={item._id} className="border-2 border-gray-500 rounded-lg px-4 py-2 m-4 relative hover:shadow-xl">
          ...
          <div className="flex justify-between items-center gap-x-2 mt-4 p-4">
            <Link to={`/book/detail/${item._id}`}><BsInfoCircle className='text-2xl text-green-800 hover:text-black'/></Link>
            <Link to={`/book/edit/${item._id}`}><AiOutlineEdit className="text-2x1 text-yellow-600 hover:text-black"/></Link>
            <Link to={`/book/delete/${item._id}`}><MdOutlineDelete className="text-2x1 text-red-600 hover:text-black"/></Link>
          </div>
        </div>
      ))}
    </div>
